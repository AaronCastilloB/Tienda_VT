...
public class RegistroServiceImpl implements RegistroService {
    ...
    public Model crear(Model model, Usuario usuario) throws MessagingException {
        ...
        if (!usuarioService.existeUsuarioPorUsernameOCorreo(usuario.getUsername(),
                usuario.getCorreo())) {
    ...
    public void habilitar(Usuario usuario, MultipartFile imagenFile) {
        ...
        if (!imagenFile.isEmpty()) {
            String ruta = firebaseStorageService.cargaImagen(imagenFile, "usuarios", usuario.getIdUsuario());
    ...
    private String demeClave() {
        String base = "ABCDEFGHIJKLMNOPQRSTUVXYZ0123456789";
        String clave = "";
        for (int i = 0; i < 40; i++) {
